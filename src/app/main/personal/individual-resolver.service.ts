...
export const IndividualResolver: ResolveFn<boolean> = () => {
  ...
  return combineLatest([store.select('individual'), store.select('auth')]).pipe(
    take(1),
    switchMap(([ind, auth]) => {
      const id = auth.user?.isRegistered;
      if (id && id === ind.actualUser?._id) {
        return of(true);
      } else if (id) {
        store.dispatch(individualActions.beginDataFetch({ id, isSelf: true }));
        return actions$.pipe(
        ...
        store.dispatch(individualActions.wouldRegister());
        return of(true);
      }
    })
  );
